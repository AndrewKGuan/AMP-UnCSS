...
const stubs = {
  ...
  'amp-experiment': () => {
     ...
     *  as defined by the experiment JSON. In theory, we could simply append
     *  each data value to the body tag to cover all bases. This would be an
     *  On^2 operation, but n would be small as it is hand written.
     */
   ...
   */
  'amp-img': (element, $) => {
    element.append('<img decoding="async" src="" class="i-amphtml-fill-content i-amphtml-replaced-content">');
    if (element.attr('layout')) {
      stubs.layout(element, $);
    }
  },
  ...
  'amp-sidebar': (element, $) => {
    ...
    if (element.contents('nav')) { // check for nav element
      ...
      navClone.each(stubChildren); // Do necessary stubbing for each child
      if ($('[toolbar-target]')) {
    ...
    /**
     * Recursive method of digging through dom tree and stubbing all children
     *  of clone as they will not be stubbed during out stub call.
     * @param {int} _ - unused index value.
     * @param {any} element - child element.
     */
    function stubChildren(_, element) {
      const elCheerio = cheerio(element, $);
      if (elCheerio.children().length > 0) {
        elCheerio.children().each(stubChildren);
      }
      if (element.type === 'tag' && stubs[element.name]) {
        stubs[element.name](elCheerio);
      }
    }
  },
