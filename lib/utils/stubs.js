 ...
 */
function stubPage($, ampFile) {
  const regX = /\'(.*)\'/g;
  let selectors = [];
  $('*').each((_, el) => {
    if (el.attribs['[class]']) {
      const dataBound = el.attribs['[class]'];
      const x = dataBound.match(regX);
      const c = x.reduce((acc, item) => {
        return acc.concat(...item.replace(/\'/g, '').split(':'));
      }, []);
  ...
  ampFile.selectorWhiteList = ampFile.selectorWhiteList
      .concat([...new Set(selectors)]);
}
