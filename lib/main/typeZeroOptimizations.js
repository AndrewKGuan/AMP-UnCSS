...
const DomParser = require('../interfaces/DomParser.js')
  ...
  _handleCommaSeparatedSelectors: function(ampFile) {
    ...
    ampFile.commaSeparatedSelectors.forEach(css=> {
      ...
      css.selectorsList.forEach(selector => {
        if(!css.isUsed) {
          // Can skip checking whole list if at least one is used.git di
          ...
          else if (ampFile.$(selector).toArray().length > 0) {
      ...
      if(!css.isUsed) {
        ...
        deferredSelectors.forEach(dS => {
          if(ampFile.$(dS.base).toArray().length > 0) {
