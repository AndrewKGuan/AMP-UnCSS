...
const typeZeroOptimizations = {
   ...
   * @private
   */
  _removeEmptySelectors: function(ampFile, selector){
  ...
  _removeUnusedPsuedos: function(ampFile) {
    return this._pseudoSelectorIterator(ampFile, 'pseudos');
  },
  _removeUnusedEscapedPsuedos: function(ampFile) {
    return this._pseudoSelectorIterator(ampFile, 'escapedPseudos');
  },
  ...
  optimize: function(ampFile) {
    ...
    if(!ampFile.hasExceptionTags()) {
      ...
      this._removeUnusedPsuedos(ampFile);
      this._removeUnusedEscapedPsuedos(ampFile) // Currently doesn't work...
