...
class AmpFile {
  ...
  constructor(filePath, options, browser) {
    ...
    if (!this._hasValidHtml(this.rawHtml)) {
      this.setFailure(performance.now(), 'Invalid HTML structure');
    }
    return this;
  }
  ...
  async dynamicDomCount(selector) {
    return await this.dynamicDom.count(selector);
  }
  ...
  async dynamicQueryAll(selector) {
    return await this.dynamicDom.queryAll(selector);
  }

  /**
   * Checks whether or not the given HTML includes relevant parts. Essentially
 *   checks that an HTML string exists and it contains the bare minimum parts.
   * @param {string} html
   * @return {boolean}
   */
  _hasValidHtml(html) {
    ...
    return searchStrings.every((str) => {
      return html.toLowerCase().includes(str.toLowerCase());
    });
  }
}
