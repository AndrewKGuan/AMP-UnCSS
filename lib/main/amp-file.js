...
class AmpFile {
  ...
  isSelectorWhitelisted(selector) {
    return this.selectorWhiteList && this.selectorWhiteList.has(selector);
  }
  ...
  async prep() {
    ...
    this._stats.inputSize = Buffer.byteLength(this.staticDom.getOriginalHtml(), 'utf8');
