...
class AmpFile {
   ...
   */
  async prep(opts, browser) {

    let rawHTML = this.type === 'streamable' ?
        String(this.rawHtml) :
        fs.readFileSync(this.filePath, 'utf-8');


    /** @description A CheerioInterface instance
    * @type {CheerioInterface}
    */
    this.staticDom = new CheerioInterface(rawHTML).init();
    this.staticDom.stubAmpImg();

    if(opts.optimizationLevel > 0) {
      this.dynamicDom = await new PuppeteerInterface(rawHTML).init(browser)
    }
  ...
  prepData(opts) {
    ...
    if(opts.optimizationLevel === 0) {
      this.staticDom.removeCustomStyles();
      this.staticDom.append('head',`<style amp-custom>${this.parsedCss.toString()}</style>`);
      this.optimizedHtml = this.staticDom.getHtml()

    } else {
      this.dynamicDom.removeCustomStyles();
      this.dynamicDom.append('head',`<style amp-custom>${this.parsedCss.toString()}</style>`);
      this.optimizedHtml = this.dynamicDom.getHtml()
    }
