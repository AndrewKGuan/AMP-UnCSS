    ...
    ampFile.commaSeparatedSelectors.forEach((css) => {
      ...
      css.selectorsList.forEach((selector) => {
        if (!css.isUsed) {
          ...
          if (DomParser._hasPseudo(selector) ||
              DomParser._hasEscapedPseudo(selector)) {
    ...
    keyframeNodes.forEach((node) => {
      ...
      ampFile.parsedCss.walkRules((rule) => {
        rule.nodes.forEach((ruleNode) => {
          if (ruleNode.value.includes(animationName)) {
            node.isUsed = true;
          }
        });
      });
      ...
      keyframeNodes.forEach((node) => {
        if (!node.isUsed) unusedKeyframes.push(node.ruleName);
      });
