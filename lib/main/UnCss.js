...
class UnCss {
  ...
  constructor(files, options, callback)
  {
    ...
    if(this.reportName) {
      if(!(this.reportName.split('.').length > 1 && this.reportName.split('.')[1] === 'json')) {
        this.reportName += '.json';
      }
    }
    ...
    this.options = Object.assign(
        defaultOptions,
        options,
        {report: !!options.reportDirectory } // If reportDirectory provided, report = True
        );
   ...
   */
  _initializeReport() {
    this.fullReportPath = this.options.reportDirectory + '/' + this.options.reportName;
