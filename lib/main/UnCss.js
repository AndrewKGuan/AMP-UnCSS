...
class UnCss {
  ...
  _initializeReport() {
    if(!fs.existsSync(this.options.reportDir)) {
      ...
      const fullReportPath = this.options.reportDir + '/' + this.options.reportBase;
      if(!fs.existsSync(fullReportPath)) {
        fs.writeFileSync(fullReportPath, JSON.stringify({initialized: Date.now()}))
      }
    }
  }

  _updateReportFile(ampFiles) {
    const fullPath = `${this.options.reportDir}/${this.options.reportBase}`
    const reportJson =  JSON.parse(fs.readFileSync(fullPath).toString());
    if (!reportJson.filesChanged) {
      reportJson.filesChanged = [];
    }
    ampFiles.forEach(file => {
      reportJson.filesChanged.push(file._stats)
    });

    fs.writeFileSync(fullPath, JSON.stringify(reportJson));
  }
}
