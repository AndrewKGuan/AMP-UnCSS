...
const typeOneOptimizations = {
  ...
  _handleCommaSeparatedSelectors: async function(ampFile) {
    await ampFile.commaSeparatedSelectors.forEach(async css=> {
      // Delete empty rules
      ...
      await css.selectorsList.forEach(async selector => {
        if(!css.isUsed) {
          // Can skip checking whole list if at least one is used.

          let selectorCount = await ampFile.dynamicDom.count(selector).catch(e => {throw e});
      ...
      if(!css.isUsed) {
        ...
        await deferredSelectors.forEach(async dS => {
          let baseCount = await ampFile.dynamicDom.count(dS.base).catch(e => {throw e});;
          if(baseCount > 0) {
