...
const typeOneOptimizer = require('./type-one-optimizations.js');
const typeZeroOptimizer = require('./type-zero-optimizations.js');
...
class AmpUncss {
   ...
   */
  async run() {
    let ampFiles = await this._setup().catch((e) => {
      throw e;
    });
    ampFiles = await this._optimize(ampFiles).catch((e) => {
      throw e;
    });
    const results = await this._tearDown(ampFiles).catch(async (err) => {
  ...
  async _optimize(ampFiles) {
    ...
    const staticOnlyGroup = ampFiles.reduce((acc, af) => {
      if (af.optimizationLevel === 0) acc.push(af);
      return acc;
    }, []);
    const dynamicGroup = ampFiles.reduce((acc, af) => {
      if (af.optimizationLevel === 1) acc.push(af);
      return acc;
    }, []);

    const failureGroup = ampFiles.reduce((acc, af) => {
      if (af.hasFailed()) acc.push(af);
      return acc;
    }, []);

    const staticGroupOptimized = staticOnlyGroup.map((af) => {
      return typeZeroOptimizer.optimize(af);
    });

    const dynamicGroupOptimized = await Promise.all(dynamicGroup.map((af) => {
    ...
    return [...staticGroupOptimized, ...dynamicGroupOptimized, ...failureGroup];
  }
  ...
  async _tearDown(ampFiles) {
    ...
    await Promise.all(ampFiles.map((af) => {
      return af.rewriteHtmlWithNewCss()
          .then(async (af) => {
            af.getCompletionStats();
            return await af.closePage();
          })
          .catch((err) => {
            console.log(
                'Error occurred during AmpFile.rewriteHtmlWithNewCss Error: ',
                err);
            throw err;
          });
    })).catch((err) => {
      console.log('Error occurred during AmpUncss._teardown(): ', err);
    });
    ...
    if (this.options.streamable) {
      ...
      if (ampFiles[0]._stats.status.failed) {
        return new Error('File optimization failed with error:'
            + ampFiles[0]._stats.error);
      }
      return {
