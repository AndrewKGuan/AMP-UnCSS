...
class AmpUncss {
  ...
  constructor(inputFilePaths, options, callback) {
    ...
    if (this.options.selectorWhiteList) {
      this.options.selectorWhiteList = new Set(this.options.selectorWhiteList);
    }
   ...
   */
  async run() {
    if (!this.options.streamable) console.log('Setting up process...');
  ...
  async executor(ampFiles) {
    ...
    let promiseChain = Promise.resolve(),
        progressBar = null;
    if (this.options.streamable === false) {
      ...
      promiseChain = promiseChain
          .then(() => {
            return Promise.all(
                ...
                .then(ampFiles => {
                  if(progressBar) progressBar.increment(1000/(count / batchSize));
                  return ampFiles
                });
          })
    }
