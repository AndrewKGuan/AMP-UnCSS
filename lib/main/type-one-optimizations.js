...
const typeOneOptimizations = {
    ...
    return Promise.all(
        Array.from(ampFile.commaSeparatedSelectors).map(async (css) => {
          ...
          await Promise.all(css.selectorsList.map(async (selector) => {
            ...
            } else if (await ampFile.dynamicDomCount(selector) > 0) {
              css.isUsed = true;
            }
            return selector;
          }));
    ...
    keyframeNodes.forEach((node) => {
      ...
      ampFile.parsedCss.walkRules((rule) => {
        rule.nodes.forEach((ruleNode) => {
          ...
          if (!(ruleNode.value && ruleNode.value.includes(animationName))) node.isUsed = true;
        });
      });
      ...
      if ( prefixedSelectorCount === 0) {
        rules.walkRules(prefixedSelector, (node) => {
          if (node.selector === prefixedSelector) {
            if (!ampFile.isSelectorWhitelisted(prefixedSelector)) {
              node.remove();
  ...
  optimize2: async function(ampFile) {
    const html = await ampFile.dynamicDom.page.content();
    console.log(html)
