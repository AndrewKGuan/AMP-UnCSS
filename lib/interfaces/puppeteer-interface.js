...
class PuppeteerInterface {
  ...
  /**
   * Utilize Cheerio.js methods to remove any <style ...> tags from the
   *    HTML document. This implementation assumes that only two <style ...>
   *    tags exist on a given HTML page - <style amp-boilerplate /> and
   *    <style amp-custom>;
   * @return {Promise<void>}
   */
  async removeCustomStyles() {
    await this.page.evaluate(() => {
      return new Promise((resolve) => {
        ...
        .catch((err) => {
          console.log('Error during PuppeteerInterface.removeCustomStyles: ' +
              err);
          throw err;
        });
  }
  ...
  /**
   * Add a new <style amp-custom> tag containing the optimized CSS to the
   *    head of the current Puppeteer.Page instance.
   * @param {string} cssStyleString
   * @return {Promise<boolean>}
   */
  async appendCustomStyle(cssStyleString) {
    await this.page.evaluate((newStyleString) => {
      return new Promise((resolve) => {
        const head = document.head || document.getElementsByTagName('head')[0];
        const style = document.createElement('style');
        style.setAttribute('amp-custom', '');
        style.type = 'text/css';
        style.innerHTML = newStyleString;
        head.appendChild(style);
        resolve(!!document.querySelector('style[amp-custom=""]'));
      });
    }, cssStyleString)
        .catch((err) => {
          console.log('Error during PuppeteerInterface.appendCustomStyles: '
          + err);
          throw err;
        });
  }
