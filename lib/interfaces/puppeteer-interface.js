...
class PuppeteerInterface {
  ...
  async count(selector) {
    if (typeof selector === 'string' && selector.length > 0) {
      ...
      throw Error(`Count() expects input type of string with non-zero length. Input '${selector}' is not string or is an empty string.`);
    }
  }
  ...
  async getHtml() {
    return await this.page.evaluate(() => {
      return new Promise((resolve) => {
        ...
        .catch((err) => {
          console.log('Error during PuppeteerInterface.getOriginalHtml: ' + err);
          throw err;
        });
  }
  ...
  async clickAllAmpAccordionSections() {
    return this.page.$$eval('amp-accordion', (accordions) => {
      return new Promise((resolve) => {
        ...
        // many nested dynamic calls there are. timeout=350 seems to be a
        // minimum to get testing to pass.
        setTimeout(() => {
          resolve(true);
        }, 350);
      });
