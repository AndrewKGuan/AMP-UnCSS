...
const stubDom = require('../utils/stubs');
...
class CheerioInterface {
  ...
  init(ampFile) {
    ...
    this.stubbedPage = cheerio.load(
        this.html
            .replace(/<template/g, '<amp-template')
            .replace(/<\/template/g, '</amp-template'));
    stubDom(this.stubbedPage, ampFile);
    return this;
  }
  ...
  queryAll(selectors) {
    try {
      return this.stubbedPage(selectors).toArray();
    } catch (err) {
      throw Error(e);
    }
  }
  ...
  count(selectors) {
    try {
      return this.stubbedPage(selectors).toArray().length;
    } catch (e) {
      throw Error(e);
    }
  }
   ...
   * @param {string} html
   */
  appendOriginal(contextSelector, html) {
  ...
  /**
   * Get raw html string from the original Cheerio DOM
   * @return {string}
   */
  getOriginalHtml() {
  ...
  /**
   * Get raw html string from the stubbedCheerio DOM
   * @return {string}
   */
  getStubbedHtml() {
    return this.stubbedPage.html();
  }
  ...
  insertAmpStubs(ampFile) {
    ...
    this.page('*').each((i, elem) => {
      ...
      if (elem.type==='tag' && stubElements[elem.name]) {
        ...
        stubs[elem.name](CheerioInterface.createNode(elem), this.page);
      }
    });
  }
   ...
   * @function
   * @static
   * @param {string} str - descriptor of new node
   * @return {* | * | * | * | *}
   */
