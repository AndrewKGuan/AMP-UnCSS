...
let testOpts = {
    ...
    staticDomHtmlPath = 'tests/selectors/static/staticDom.html',
    dynamicDomHtmlPath = 'tests/selectors/dynamic/dynamicDom.html',
    filePaths = [staticDomHtmlPath, dynamicDomHtmlPath ];
...
describe('unCss functions',async function() {
    ...
    await it(`should ${testOpts ? '' : 'not'} init with a Puppeteer browser`, async function(){
      await uncss.init();
    ...
    describe('#_optimize', async function(){
      ...
      it('should optimize each ampFile', function(){
        assert.ok(ampFiles.every(af => Object.keys(af._stats.selectorsRemoved).length  > 0))
      });

      it('should optimize each ampFile with the correct optimization', function(){
