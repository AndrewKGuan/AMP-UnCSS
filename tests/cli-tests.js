...
describe('AMP UnCSS CLI', function() {
  ...
  it('should respond to defined optimization level', async function() {
    ...
    const report = JSON.parse(fs.readFileSync(
        'reports/amp_uncss_report.json', 'utf-8'));
    assert.ok(report.tests.length === 3 &&
        report.tests[0].options.optimizationLevel === 0);
    assert.ok(report.tests.length === 3 &&
        report.tests[1].options.optimizationLevel === 1);
    assert.ok(report.tests.length === 3 &&
        report.tests[2].options.optimizationLevel === 2);
  });
